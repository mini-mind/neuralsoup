...
模型编辑器是 neuralsoup 项目中的独立拓扑沙盒，用于设计和编辑局部神经网络拓扑，并观察独立的脉冲可视化效果。

## 核心功能

### 1. 网络组件
- **输入节点（感受器）**：当前仅提供视觉输入接点
...
- **左键拖拽**：框选多个神经元或拖拽神经元
- **Ctrl+左键**：多选神经元
- **Ctrl+拖拽节点**：创建连接（神经元目标会显示 hover 高亮）
- **右键拖拽**：平移画布
- **Delete键**：删除选中元素
- **滚轮**：缩放画布视图
...
3. **创建连接**：Ctrl+拖拽节点到目标（神经元目标会显示黄色高亮）
4. **调整视图**：右键拖拽平移，滚轮缩放
5. **删除元素**：选中后按Delete键
...
2. **hover高亮不显示**：当前只有神经元目标会显示 hover 高亮，效应器和感受器不会显示同类提示
