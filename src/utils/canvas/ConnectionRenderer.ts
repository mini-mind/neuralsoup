...
export class ConnectionRenderer {
  ...
  private static getConnectionStartPoint(
    ...
    if (connecting.fromType === "node") {
      ...
      const fromReceptor = context.receptors.find(
        (r) => r.id === connecting.from,
      );
      if (fromReceptor) {
        const receptorIndex = context.receptors.indexOf(fromReceptor);
        const receptorSpacing =
          context.receptorX / Math.max(1, context.receptors.length);
        fromX =
          context.receptorX +
          receptorIndex * receptorSpacing +
          receptorSpacing / 2;
        fromY = context.receptorY + 30;
      }
    } else if (connecting.fromType === "effector") {
      const fromEffector = context.effectors.find(
        (e) => e.id === connecting.from,
      );
      if (fromEffector) {
        const effectorIndex = context.effectors.indexOf(fromEffector);
        const effectorSpacing =
          context.effectorX / Math.max(1, context.effectors.length);
        fromX =
          context.effectorX +
          effectorIndex * effectorSpacing +
          effectorSpacing / 2;
