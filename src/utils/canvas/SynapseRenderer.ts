...
export class SynapseRenderer {
  ...
  static draw(ctx: CanvasRenderingContext2D, props: SynapseRenderProps): void {
    props.synapses.forEach((synapse) => {
      ...
      if (isSelfConnection) {
        this.drawSelfConnection(
          ctx,
          fromX,
          fromY,
          synapse,
          props.selectedSynapse,
        );
      } else {
        this.drawNormalConnection(
          ctx,
          fromX,
          fromY,
          toX,
  ...
  private static getSynapseEndpoints(
    ...
    if (fromNode) {
      ...
      const receptorSpacing =
        props.receptorX / Math.max(1, props.receptors.length);
      fromX =
        props.receptorX + receptorIndex * receptorSpacing + receptorSpacing / 2;
    ...
    if (toNode) {
      ...
      const effectorSpacing =
        props.effectorWidth / Math.max(1, props.effectors.length);
      toX =
        props.effectorX + effectorIndex * effectorSpacing + effectorSpacing / 2;
      toY = props.effectorY;
    }
