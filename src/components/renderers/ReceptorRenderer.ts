...
export class ReceptorRenderer {
  static draw(
    ...
    receptors.forEach((receptor) => {
      ...
      // 只绘制视觉模态（简化后的设计）
      const visionModality = receptor.modalities.find(
        (m) => m.type === "vision",
      );

      if (visionModality) {
        this.drawSimplifiedVisionModality(
          ctx,
          visionModality,
          receptorX,
          receptorY,
  ...
  private static drawSimplifiedVisionModality(
    ...
    if (visionModality.inputs && visionModality.inputs.length > 0) {
      visionModality.inputs.forEach((input: any) => {
        ...
        if (
          inputX >= receptorX - 12 &&
          inputX <= receptorX + receptorWidth + 12
        ) {
          // 绘制输入节点
          ctx.beginPath();
  ...
  private static drawScrollIndicator(
    ...
    if (contentWidth > receptorWidth) {
      ...
      const scrollProgress = Math.max(
        0,
        Math.min(1, scrollX / scrollableWidth),
      );
      const thumbWidth = Math.max(
        20,
        (receptorWidth / contentWidth) * (receptorWidth - 10),
      );
      const thumbX =
        receptorX + 5 + scrollProgress * (receptorWidth - 10 - thumbWidth);
  ...
  private static drawVisionModality(
    ...
    ctx.textAlign = "center";
    ctx.fillText(
      visionModality.label,
      currentX + visionWidth / 2,
      receptorY + 16,
    );
  ...
  private static drawOtherModality(
    ...
    ctx.fillText(
      modality.label,
      modalityX + otherModalityWidth / 2,
      receptorY + 16,
    );
