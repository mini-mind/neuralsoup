...
export const useSNNTopologyEvents = ({
  canvasRef,
  state,
}: UseSNNTopologyEventsProps) => {
  const {
    nodes,
    synapses,
    receptors,
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (e.button === 2) {
        ...
        if (clicked) {
          if (
            clicked.type === "receptor-tab" &&
            clicked.modalityIndex !== undefined
          ) {
          ...
          const clickedSynapse = CanvasEventHandler.detectClickedSynapse(
            x,
            y,
            synapses,
            {
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent<HTMLCanvasElement>) => {
      ...
      const isInReceptorArea =
        y >= receptorY && y <= receptorY + receptorHeight;
