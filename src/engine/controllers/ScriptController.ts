...
export class ScriptController {
   ...
   */
  private buildActionObjectForComputation(
    agent: Agent,
    deltaTime: number,
  ): any {
    return {
      move: (direction: [number, number, number, number?]) => {
        ...
        const [forward, left, right, backward = 0] = direction.map((val) =>
          Math.max(0, Math.min(1, Number(val) || 0)),
        );
        // 保存脚本计算结果而不是直接应用
   ...
   * 构建agent对象 - 用于直接应用模式
   */
  private buildAgentObjectForDirectApplication(
    agent: Agent,
    deltaTime: number,
    applyActionCallback: (
      agent: Agent,
    ...
    const state = this.buildStateObject(agent);
    const action = this.buildActionObject(
      agent,
      deltaTime,
      applyActionCallback,
    );
