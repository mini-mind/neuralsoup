...
export class CorticalColumn {
  ...
  forward(inputs: number[]): number[] {
    if (inputs.length !== this.inputSize) {
      throw new Error(
        `输入大小不匹配: 期望 ${this.inputSize}, 得到 ${inputs.length}`,
      );
    }
    ...
    // 返回输出层的活动（简化为速率编码）
    const outputLayer =
      this.membranePotentials[this.membranePotentials.length - 1];
    const outputs = outputLayer.map((v) => {
      // 将膜电位映射到[0,1]范围，并增强信号
      const normalized = Math.max(
        0,
        (v - this.V_REST) / (this.V_THRESHOLD - this.V_REST),
      );
  ...
  private updateMembranePotentials(
    layerIdx: number,
  ...
  resetState(): void {
    ...
    for (
      let layerIdx = 0;
      layerIdx < this.membranePotentials.length;
      layerIdx++
    ) {
  ...
  getNetworkInfo(): any {
    return {
      ...
      totalParameters: this.weights.reduce(
        (sum, layer) =>
          sum + layer.reduce((layerSum, neuron) => layerSum + neuron.length, 0),
        0,
      ),
    };
  }
