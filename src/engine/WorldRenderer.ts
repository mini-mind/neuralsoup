...
export class WorldRenderer {
  ...
  private initializeRenderers(): void {
    ...
    this.visionRenderer = new VisionRenderer(
  ...
  private updateCamera(): void {
    ...
    // 平滑插值pivot点
    this.worldContainer.pivot.x +=
      (targetPivotX - this.worldContainer.pivot.x) * this.cameraLerpFactor;
    this.worldContainer.pivot.y +=
      (targetPivotY - this.worldContainer.pivot.y) * this.cameraLerpFactor;
    ...
    // 平滑插值旋转 (处理角度的周期性)
    let shortestAngle =
      ((targetRotation - this.worldContainer.rotation + Math.PI * 3) %
        (Math.PI * 2)) -
      Math.PI;
