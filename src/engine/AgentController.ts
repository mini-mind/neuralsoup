...
export class AgentController {
  ...
  public createCorticalColumn(agentId: number, visionCells: number = 36): void {
    if (!this.corticalColumns.has(agentId)) {
      ...
      console.log(
        `为智能体 ${agentId} 创建了皮质柱，视觉输入维度：${inputSize}`,
      );
    }
  }
   ...
   * 更新皮质柱配置
   */
  public updateCorticalColumnConfiguration(
    agentId: number,
    visionCells: number,
  ): void {
  ...
  public updateAgent(agent: Agent, deltaTime: number): void {
    ...
    if (
      this.enablePlayerInputInScript &&
      currentTime - this.lastKeyboardCheckTime > this.keyboardCheckInterval
    ) {
  ...
  private updateScriptAgent(agent: Agent, deltaTime: number): void {
    // 先获取脚本计算结果
    const scriptOutput = this.scriptController.computeScriptOutput(
      agent,
      deltaTime,
    );
    ...
    if (this.enablePlayerInputInScript && this.hasRecentKeyboardInput) {
      ...
      if (keyboardInputs[1] > 0) {
        // 手动前进
        // 如果脚本和手动都是前进，取较大值
        netForward = Math.max(netForward, keyboardInputs[1]);
      } else if (keyboardInputs[3] > 0) {
        // 手动后退
        // 手动后退应该减少前进速度，实现抵消效果
